...
const logicTreeNodes = {
    ...
    hard_reboot: { question: "Hold the Power button for 10 seconds to force shut down. After powering back on, is it resolved?", options: [ { label: "Yes", resolution: "Issue Resolved. Forced shut down cleared the freeze.", type: "success" }, { label: "No", next: "factory_reset", icon: "ph-x" } ] },
    factory_reset: { question: "The device requires a Factory Reset via the boot menu. Have you performed this step?", options: [ { label: "Yes, issue resolved", resolution: "Issue Resolved via Factory Reset.", type: "success" }, { label: "Yes, but issue persists", next: "rma_software", icon: "ph-warning" } ] },
    ...
    rma_software: { resolution: "Persistent software corruption despite factory reset. Prepare device for RMA.", type: "error" },
