 ...
 */
async function fetchSpreadsheetRows(): Promise<string[][]> {
  try {
    ...
    throw new functions.https.HttpsError(
      "internal",
      "Failed to access the secure credentials spreadsheet: " + err.message
    );
  }
}
...
export const requestVerificationCode = functions
  ...
  .https.onCall(async (data: { email: string; firstName: string; lastName: string }, context) => {
...
export const migrateCustomClaims = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!isAdmin && !isLegacyAdmin) {
      functions.logger.error(
        `Security Event: Unauthorized attempt to run migration. ` +
        `UID: ${uid} (${userEmail})`
      );
      throw new functions.https.HttpsError(
        "permission-denied",
...
export const deleteUserAccount = functions
  ...
  .https.onCall(
    withRequiredRole("admin", async (data: { email: string }, context) => {
      const { email } = data;

      if (!email || typeof email !== "string" || !email.includes("@")) {
        throw new functions.https.HttpsError(
          "invalid-argument",
          "A valid email address is required for deletion."
        );
      }
      ...
      try {
        ...
        throw new functions.https.HttpsError(
          "internal",
          "Failed to completely delete user account: " + err.message
        );
      }
    })
  );
