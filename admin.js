    ...
    updateLiveFleetStatus: function() {
        try {
            ...
            Object.values(this.usersCache).forEach(u => {
                ...
                if (u.dynamicDevices && Array.isArray(u.dynamicDevices)) {
            ...
            console.error('[updateLiveFleetStatus] Error rendering live fleet status:', error);
            const container = document.getElementById('live-fleet-status-container');
            if (container) {
                container.innerHTML = `
                    <div style="color: var(--danger); padding: 16px; border: 1px solid var(--danger); border-radius: var(--radius-md); font-size: 13px;">
                        <strong>Error rendering live fleet status command center:</strong> ${error.message}
                    </div>
                `;
            }
        }
